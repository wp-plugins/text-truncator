 ...
 */
$_sMaingPluginFileName  = 'text-truncator.php';
if ( file_exists( dirname( __FILE__ ). '/' . $_sMaingPluginFileName ) ) {
   include( $_sMaingPluginFileName );
}

if ( ! class_exists( 'TextTruncator_Registry' ) ) {
    return;
}

// 1. Delete transients
$_aPrefixes = array(
    TextTruncator_Registry::TRANSIENT_PREFIX, // the plugin transients
...
// 2. Delete options
$_aOptions = get_option( TextTruncator_Registry::$aOptionKeys[ 'setting' ], array() );
...
array_walk_recursive( 
    TextTruncator_Registry::$aOptionKeys, // subject array
